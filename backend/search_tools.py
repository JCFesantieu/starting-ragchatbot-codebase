...
class CourseOutlineTool(Tool):
    """Tool for retrieving complete course outline with all lessons"""
    ...
    def execute(self, course_name: str) -> str:
        ...
        Args:
            course_name: The course name to get outline for
        ...
        # Step 1: Resolve the course name using fuzzy matching
        actual_course_title = self.store._resolve_course_name(course_name)

        if not actual_course_title:
            return f"No course found matching '{course_name}'"

        # Step 2: Get course metadata from the catalog
        try:
            results = self.store.course_catalog.get(ids=[actual_course_title])

            if not results or not results.get('metadatas') or not results['metadatas']:
                return f"Course '{actual_course_title}' found but no metadata available"

            metadata = results['metadatas'][0]

            # Step 3: Extract course information
            course_title = metadata.get('title', actual_course_title)
